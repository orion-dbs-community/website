    ...
    desc_section <- if (!is.na(table_desc) && table_desc != "") {
      tagList(
        div(
          ...
          # Table descriptions are markdown too, render them as such. This
          # needs dangerouslySetInnerHTML (via raw_html_tag()), not HTML() --
          # see bq_description.R for why.
          raw_html_tag(bq_description_html(table_desc), class = "description-content"),
          tags$div(
      ...
      reactable(
        ...
        details = function(schema_index) {
          ...
          if (!is.null(nested_data)) {
            # Recursive function to render nested fields at any depth
            render_nested_fields <- function(data, depth = 1) {
              div(
                class = "nested-fields-container",
                div(class = "nested-fields-header", paste0("Nested Fields", if(depth > 1) paste0(" (Level ", depth, ")") else "")),
                reactable(
                  data,
                  pagination = FALSE,
                  outlined = TRUE,
                  ...
                  details = function(nested_index) {
                    deeper_nested_data <- data$nested_fields[[nested_index]]
                    if (!is.null(deeper_nested_data)) {
                      render_nested_fields(deeper_nested_data, depth + 1)
                    }
                  },
                  class = "schema-table",
            ...
            render_nested_fields(nested_data)
          }
        },
        class = "schema-table",
        theme = reactableTheme(cellPadding = "8px 12px")
